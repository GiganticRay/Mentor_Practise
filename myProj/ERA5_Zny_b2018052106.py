...
# Lei ---path
input_path  = '/home/lei/Document/Proj/Liu_Task/data/ncep2018/ERA5-201801.nc'
...
def FitIsoContourToEllipse(ax, isomorph, data_set, specified_values=[], l_region_restriction=[]): 
    ...
    if(len(l_region_restriction) != 0):
        l_region_restriction_cp = l_region_restriction.copy()
        l_region_restriction_cp.append(l_region_restriction_cp[0])

        for i in range(len(l_region_restriction_cp)-1):
            ax.add_line(Line2D([l_region_restriction_cp[i][0], l_region_restriction_cp[i+1][0]], [l_region_restriction_cp[i][1], l_region_restriction_cp[i+1][1]], color='red')) 
