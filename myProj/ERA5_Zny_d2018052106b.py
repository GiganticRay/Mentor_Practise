...
#/mnt/    ---path 
#input_path  = '/mnt/e/PythonPro/XNW/ERA5_Data/2018/ERA5-201805.nc'
#output_dir  = '/mnt/e/PythonPro/XNW/ERA5/png2018/201805/'

#/Zny/    ---path  
# input_path  = '/public/home/chengc/xnw/ERA5_Data/2018/ERA5-201805.nc'
# output_dir  = '/public/home/chengc/xnw/ERA5/png2018/201805/'

# Lei ---path
input_path  = '/home/lei/Document/Proj/Liu_Task/data/ncep2018/ERA5-201805.nc'
...
def ConstructBasicMapElem(ax): 
    ...
    #/mnt/    --path	
    #reader = Reader("/mnt/e/PythonPro/Region/province.shp")
    ...
    # 设定显示经纬度范围 
    img_extent = [start_map_lon,end_map_lon,start_map_lat,end_map_lat]     # south-west region of China, [-180,180,-90,90]  # World Region     
    ax.set_extent(img_extent) 
    
    #ax.set_xticks(5)
    
    ax.set_ylim([start_map_lat, end_map_lat])
...
''' 
    ...
    center_region_restriction:中心点区域限制  default为空，表示不限制；输入格式可选择 nx2 list: [p1, p2, p3...pn]. pi = [lon, lat], 由p1->p2->p3->...->pn->p1构成一个封闭多边形
    longaxisend_region_restriction:长轴端点区域限制  default为空，表示不限制；输入格式可选择 nx2 list: [p1, p2, p3...pn]. pi = [lon, lat], 由p1->p2->p3->...->pn->p1构成一个封闭多边形
''' 
def FitIsoContourToEllipse(ax, isomorph, data_set, specified_values=[], center_region_restriction=[], longaxisend_region_restriction=[]): 
    # Geodesic的计算对象 
    output_text = "" 
    np_output = [['center_lon',' ', 'center_lat',' ', 'width_axes_slop',' ', 'width_axes_dist',' ', 'height_axes_slop',' ', 'height_axes_dist',' ', 'ellipse_area']]

    # 如果是 restricted_version, 绘制出restricted_region
    if(len(center_region_restriction) != 0):
        center_region_restriction_cp = center_region_restriction.copy()
        center_region_restriction_cp.append(center_region_restriction_cp[0])

        for i in range(len(center_region_restriction_cp)-1):
            ax.add_line(Line2D([center_region_restriction_cp[i][0], center_region_restriction_cp[i+1][0]], [center_region_restriction_cp[i][1], center_region_restriction_cp[i+1][1]], color='red')) 

        for i in range(len(center_region_restriction_cp)-1):
            ax.add_patch(Ellipse(([center_region_restriction_cp[i][0], center_region_restriction_cp[i][1]]), width=0.3, height=0.3, angle=60, linewidth=0.15, fill=True, zorder=2, facecolor='yellow', edgecolor='r')) 

    # 如果是 restricted_version, 绘制出restricted_region
    if(len(longaxisend_region_restriction) != 0):
        longaxisend_region_restriction_cp = longaxisend_region_restriction.copy()
        longaxisend_region_restriction_cp.append(longaxisend_region_restriction_cp[0])

        for i in range(len(longaxisend_region_restriction_cp)-1):
            ax.add_line(Line2D([longaxisend_region_restriction_cp[i][0], longaxisend_region_restriction_cp[i+1][0]], [longaxisend_region_restriction_cp[i][1], longaxisend_region_restriction_cp[i+1][1]], color='blue')) 

        for i in range(len(longaxisend_region_restriction_cp)-1):
            ax.add_patch(Ellipse(([longaxisend_region_restriction_cp[i][0], longaxisend_region_restriction_cp[i][1]]), width=0.3, height=0.3, angle=60, linewidth=0.15, fill=True, zorder=2, facecolor='yellow', edgecolor='r')) 
            ...
            for contour_idx, ith_contour in enumerate(ith_contours): 
                ...
                if(len(ith_contour) >= 5):
                    ...
                    if(len(fit_ellipse_P.coefficients) > 0 and isinstance(fit_ellipse_P.coefficients[0], complex)==False): 
                        ...
                        # 如果对拟合椭圆中心有范围限制，中心落在之外, 则不绘制该拟合椭圆
                        if(len(center_region_restriction)!=0 and IsInsidePolygon(center, center_region_restriction) is not True):
                            continue
                        ...
                        # 如果对拟合椭圆长短轴端点超出范围限制，则不绘制该拟合椭圆
                        if(((len(longaxisend_region_restriction)!=0) and (IsInsidePolygon(width_vertex, longaxisend_region_restriction)) and (IsInsidePolygon(height_vertex, longaxisend_region_restriction))) is not True):
                            continue
                        ...
                        for p_coord in vertices: 
                            if(np.isnan(p_coord[0]) or np.isnan(p_coord[1])):
                                continue
                            #print('p_coord[0]',p_coord[0],p_coord[0].is_integer,'  ','p_coord[1]',p_coord[1],p_coord[1].is_integer)    
                            ...
                            # 注意坐标换算 
                            left_bottom_V   = data_set[end_map_lat-left_bottom_P[1], left_bottom_P[0]-start_map_lon] 
                            left_top_V      = data_set[end_map_lat-right_top_P[1], left_bottom_P[0]-start_map_lon] 
                            right_top_V     = data_set[end_map_lat-right_top_P[1], right_top_P[0]-start_map_lon] 
                            right_bottom_V  = data_set[end_map_lat-left_bottom_P[1], right_top_P[0]-start_map_lon] 
                        ...
                        ax.add_patch(Ellipse(([width_vertex[0], width_vertex[1]]), width=0.2, height=0.2, angle=60, linewidth=0.10, fill=True, zorder=2, facecolor='green', edgecolor='r')) 
                        ax.add_patch(Ellipse(([height_vertex[0], height_vertex[1]]), width=0.2, height=0.2, angle=60, linewidth=0.10, fill=True, zorder=2, facecolor='green', edgecolor='r')) 
                ...
                for lon in range(left_lower_lon, right_upper_lon, 1): 
                    for lat in range(left_lower_lat, right_upper_lat, 1): 
                        ...
                        if(contour_polygon.pointInPolygon(lon, lat)): 
                            if(data_set[end_map_lat-lat, lon-start_map_lon] < minmum_point[0]): 
                                minmum_point = [data_set[end_map_lat-lat, lon-start_map_lon], lat, lon] 
                            if(data_set[end_map_lat-lat, lon-start_map_lon] > maxmum_point[0]): 
                                maxmum_point = [data_set[end_map_lat-lat, lon-start_map_lon], lat, lon] 
 
                # 绘制mimmum_point与maxmum_point在ax上面 scatter(lon, lat) 
                ax.scatter(minmum_point[2], minmum_point[1]) 
                 
                ax.text(minmum_point[2], minmum_point[1], "[{}, {}, {}]".format(minmum_point[2], minmum_point[1], format(minmum_point[0], '.2f')),  
                    fontsize=10, color = "b", style = "italic", weight = "light", 
                    verticalalignment='up', horizontalalignment='left') 

''' 
    找出任意多边形内部的中心极值点，最大和最小嘛，方法：暴力求解， 
    1. 先遍历每一条任意多边形内部坐标，求出其左下角顶点和右上角顶点的位置，求得一个正方形区域 
    2. 遍历该正方形区域中的每一个点，判定其是否属于该区域内，属于的话记录该点的经纬度坐标并更新最大最小值 
''' 
def GetExtremPointInPolygon(ax, curr_polygon, data_set): 
    contour_P_X = []
    contour_P_Y = []
    if(len(curr_polygon) !=0): 
        for i in curr_polygon:
            contour_P_X += [i[0]]
            contour_P_Y += [i[1]]
        #print(contour_P_X)
        #print(contour_P_Y)
        ...
        # 该等势线得多边形的对象，https://github.com/StephewZ/PointInPolygon 
        in_polygon     = Wall(len(contour_P_X), contour_P_X, contour_P_Y) 
        in_polygon.precalc_values() 
        # 根据地图特性，左下角上去整，右上角下取整 
        right_upper_lon     = int(max(contour_P_X))  
        right_upper_lat     = int(max(contour_P_Y)) 
        left_lower_lon      = math.ceil(min(contour_P_X))     
        ...
        print ('right_upper_lon,right_upper_lat,left_lower_lon,left_lower_lat')		
        print (right_upper_lon,right_upper_lat,left_lower_lon,left_lower_lat)
		
     
        right_upper_lon_index  = int((right_upper_lon-start_data_lon)/0.25)
        right_upper_lat_index  = int((right_upper_lat-start_data_lat)/0.25)
        left_lower_lon_index   = int((left_lower_lon-start_data_lon)/0.25)
        left_lower_lat_index   = int((left_lower_lat-start_data_lat)/0.25)

        print ('right_upper_lon_index,right_upper_lat_index,left_lower_lon_index,left_lower_lat_index')		
        print (right_upper_lon_index,right_upper_lat_index,left_lower_lon_index,left_lower_lat_index)

        # 进行区域内点的判定，并更新最大最小值及其位置 
        minmum_point        = [sys.float_info.max, 0, 0] 
        maxmum_point        = [sys.float_info.min, 0, 0] 
        for ilon in range(left_lower_lon_index, right_upper_lon_index, 1): 
            for ilat in range(left_lower_lat_index, right_upper_lat_index, 1): 
                # 如果该点在等势线多边形内部，才计算 
                dlon=ilon*0.25+start_data_lon
                dlat=ilat*0.25+start_data_lat
                if(in_polygon.pointInPolygon(dlon, dlat)): 
                    if(data_set[ilon, ilat] < minmum_point[0]): 
                        minmum_point = [data_set[ilon, ilat], dlon, dlat] 
                    if(data_set[ilon, ilat] > maxmum_point[0]): 
                        maxmum_point = [data_set[ilon, ilat],dlon,dlat] 
 
        # 绘制mimmum_point与maxmum_point在ax上面 scatter(lon, lat) 
        #ax.scatter(minmum_point[2], minmum_point[1]) 
        ax.add_patch(Ellipse(([minmum_point[1], minmum_point[2]]), width=0.3, height=0.3, angle=60, linewidth=0.15, fill=True, zorder=2, facecolor='r', edgecolor='yellow'))          

        ax.text(minmum_point[1], minmum_point[2], "[{}, {}, {}]".format(minmum_point[1], minmum_point[2], format(minmum_point[0], '.2f')),  
...
def PlotGridValue(ax, data_set): 
    for curr_lon in range(start_map_lon, end_map_lon, 1): 
        for curr_lat in range(start_map_lat, end_map_lat, 1): 
            curr_val = data_set[end_map_lat-curr_lat, curr_lon-start_map_lon] 
...
if __name__ == "__main__": 
    ...
    for it in range(81,82):
        ...
        #center_region_restriction=[[103.0, 30.0], [105.5, 33.0], [111.0, 31.0], [105.0, 27.0]]
        center_region_restriction=[[103.0, 30.0], [105.5, 33.0], [110.0, 32.0], [110.0, 30.0], [107.5, 28.0], [105.0, 27.0], [103.5, 28.0]]
        
        longaxisend_region_restriction=[[102.5, 26.0], [100.0, 30.0], [102.5, 35.0], [112.5, 35.0], [112.5, 26.0]]

        # # 获取任意多边形内部点极值
        GetExtremPointInPolygon(restricted_ax, center_region_restriction, HGTprs_850)
	
        np_ellipse_restricted_info = FitIsoContourToEllipse(restricted_ax, isoHGT, HGTprs_850, HGTprs_850_SC_Ellipse_List, center_region_restriction, longaxisend_region_restriction)
        ...
        #绘制流场
        restricted_ax.streamplot(lons[::], lats[::], UGRD_850[::,::], VGRD_850[::,::], linewidth=0.3, color='gray')  #绘制流线
        ...
        #plt.figure("normal")
        #plt.savefig(output_dir + 'ERA5_'+str_curr_time + ".png")  
			
        if(np_ellipse_restricted_info.shape[0] > 1):
            str_ellipseInfo_restricted_path = output_dir + str_curr_time + "b" + ".csv"
            np.savetxt(str_ellipseInfo_restricted_path, np_ellipse_restricted_info, delimiter=",", fmt='%s')
            plt.figure("restricted")
            plt.savefig(output_dir + 'ERA5_'+str_curr_time + "b" + ".png") 
        else :
            plt.figure("restricted")
            plt.savefig(output_dir + 'ERA5_'+str_curr_time  + ".png")           		